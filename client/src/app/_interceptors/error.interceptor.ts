      ...
      catchError(error => {
        if(error){
          ...
          switch (error.status) {
              ...
              case 500:
                const navigatonExtras: NavigationExtras = {state: {error: error.error}};
                this.router.navigateByUrl('/server-error', navigatonExtras);
                this.toastr.error(error.statusText === 'OK' ? 'Generic Error Response' : error.statusText, error.status);
                break;
